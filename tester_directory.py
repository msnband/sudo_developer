...
print("\nOpen PuTTY to se the server page...\n")
async def send_back(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
    ...
    print(message)
    writer.write(f"\n\r{message}".encode())
    writer.write("\n\n\r ************************** WELLCOME TO PYTHON SERVER **************************".encode())

    Flag = True
    while Flag == True:
        writer.write("\n\n\rYou are connected to SERVER...".encode())
        writer.write("\n\n\r>>To see the available commands and options, press 'commands >>".encode())
        data = await reader.readline()
        message = data.decode().strip()

        if message == 'commands':
            writer.write("\n\n\r>> Command 'change_folder' to move the current working directory.[change_folder <name>]".encode())
            writer.write("\n\n\r>> command 'list' to print all files and folders in the current working directory.[list]".encode())
            writer.write("\n\n\r>> Command 'read_file' to read data from the file <name> in the current working directory.[read_file <name>]".encode())
            writer.write("\n\n\r>> Command 'write_file' to write the data in <input> to the end of the file <name> in the current working directory.[write_file <name> <input>]".encode())
            writer.write("\n\n\r>> Command 'create_folder' to create a new folder with the specified <name> in the current working directory.[create_folder <name>]".encode())
            writer.write("\n\n\r>> command 'register' to register a new user and password with optional privilage.[register <username> <password> <privileges>]".encode())
            writer.write("\n\n\r>> Command 'login' to log in server.[login <username> <password>]".encode())
            writer.write("\n\n\r>> Command 'delete' to delete the user conforming with <username> from the server.[delete <username> <password>]".encode())
            writer.write("\n\n\r>> Command 'quit' to leave your directory and close the page.[quit]".encode())
        ...
        if message == "login":
            ...
            if not os.path.exists(user_name +".txt"):
                ...
                if f[0] == user_password:
                    status = True
                    os.path.chdir("my_assignment_3\Root\Admin\Current_Users")
        ...
        if message == 'list':
            ...
            for items in os.listdir("."):
                writer.write(f'\n\r{items}   created       {CreationTime}   size   {FileSize}KB\n'.encode())
        ...
        if message == "delete":
            writer.write("Insert the username you want to delete >> ".encode())
            data = await reader.readline()
            user_name = data.decode().strip()
